...
El ecosistema está desplegado sobre infraestructura de **AWS**, orquestado mediante **Apache Airflow** corriendo en contenedores **Docker** con transformaciones en **Apache Spark**. Para la parte de streaming se utilizaron conectores de **Airbyte** y sistema de mensajeria **Apache Kafka**.
...
1.  **Ingesta (Extract):**
    ...
    - **Streaming:** Consumo de API de clima on real-time a traves de **Kafka**
2.  **Carga (Load):**
...
- **Infraestructura Cloud (AWS):**
  - **EC2:**:
    ...
    - Instancia Ubuntu (4GB RAM) para el despliegue de Kafka.
...
├── kafka/                     		# AVANCE5 EXTRA-CREDIT: Lógica de Streaming
│   ├── docker-compose.yml
│   └── producer_kafka.py      		# Productor de streams
├── scripts/
...
│   ├── streaming_kafka_to_s3.py 	# Conexion Spark Streaming
│   └── weather_s3_silver.py      	# Validacion, unificacion historico+stream (raw) para carga de carpeta Silver
├── requirements.txt
├── .gitignore
└── README.md
```
